...
class ExpenseTrackerUI:
        ...
        if category:
            ...
            if amount_str:
                try:
                    ...
                    self.money -= amount
                    self.history.append(["expense", category, amount, self.money])
                    self.save_data()
                    self.update_balance()
        ...
        if source:
            ...
            if amount_str:
                try:
                    ...
                    if amount < 0: raise ValueError
                    self.money += amount
                    self.history.append(["income", source, amount, self.money])
                    self.save_data()
                    self.update_balance()
    ...
    def show_history(self):
        ...
        self.tree = ttk.Treeview(tree_frame, columns=("ID", "Type", "Category", "Amount", "Balance"), show="headings", yscrollcommand=scrollbar.set)
    ...
    def refresh_tree(self):
        ...
        for i, item in enumerate(self.history):
            self.tree.insert("", tk.END, iid=str(i), values=(i+1, item[0].capitalize(), item[1], f"${item[2]}", f"${item[3]}"))
